...
class TorchVision_DPN_68_V2():
    def __init__(self):
        sess_options = ort.SessionOptions()
        sess_options.enable_profiling = True
        self.session = ort.InferenceSession('./mlmodelscope/onnxruntime_agent/model_files/dpn68b-imagenet.onnx', sess_options)
        self.input_name = self.session.get_inputs()[0].name
        self.output_name = self.session.get_outputs()[0].name
        self.model = onnx.load('./mlmodelscope/onnxruntime_agent/model_files/dpn68b-imagenet.onnx')
